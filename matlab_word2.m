...
Content = Document.Content;% 设定光标位置从头开始
Selection = Word.Selection;
Paragraphformat = Selection.ParagraphFormat;

Document.PageSetup.TopMargin = 60; % 设定页面大小% 单位像素
Document.PageSetup.BottomMargin = 45;
Document.PageSetup.LeftMargin = 45;
Document.PageSetup.RightMargin = 45;

% Content.InsertParagraphAfter;% 接着插入
% Content.Collapse=0; % 0为不覆盖
Content.Start = 0;

title = '试 卷 分 析';
Content.Text = title;
Content.Font.Size = 16 ;%三号字
...
xueqi = '（ 2009 — 2010 学年 第一学期）定义开始的位置为上一段结束的位置';
Selection.Text = xueqi;
Selection.Font.Size = 12;
Selection.Font.Bold = 0; 
Selection.Font.ColorIndex = 'wdGreen'; % 设置字体颜色
paragraphformat.Alignment = 'wdAlignParagraphCenter';

paragraphFormat.LineSpacingRule = 'wdLineSpace1pt5'; % 设置行间距为1.5倍行距，第1段 
paragraphFormat.FirstLineIndent = 28; % 首行缩进28磅，也就是当前14磅字符的2个字符，第二段
...
Selection.TypeParagraph;%两个一起就孔一行




Selection.Font.Size = 10.5;
Tables = Document.Tables.Add(Selection.Range,12,9);%建立一个12*9的表格
...
for i = 1:9
DTI.Columns.Item(i).Width = column_width(i);%循环设置列宽
end

for i = 1:12
DTI.Rows.Item(i).Height = row_height(i);%循环设置列宽
end
...
% Selection.Range.PasteSpecial;% 将图形粘贴到当前文档里（表格的第8行第1列的单元格里），并设置图形版式为浮于文字上方
